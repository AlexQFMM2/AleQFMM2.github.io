...
void DatabasePool::createPool(size_t size) {  
    ...
    for (size_t i = 0; i < size; ++i) {  
        ...
        //schema:模式
        //在许多数据库系统中，模式是定义数据库对象（如表、视图、存储过程等）结构和组织的一种方式。
...
bool DatabaseManager::execSQL(const std::string& SQL, const std::vector<std::any>& params, TABLE& result) {  
    try {  
        // 获取连接  
        std::unique_ptr<sql::Connection> con = pool.getConnection();  
...
int main() {  
    ...
    //vector<any> vars = {any_cast<string>("user1") , any_cast<string>("a12345")};   
