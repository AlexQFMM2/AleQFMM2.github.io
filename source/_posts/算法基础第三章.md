...
void bfs(int n,int m){
    ...
    while(!q1.empty()){
        ...
        for(int i=0;i<4;i++){
            ...
            if(visited[nextX][nextY] || gameMap[nextX][nextY] == 1) continue; // 如果已访问或是障碍物，跳过

            dp[nextX][nextY] = min(dp[nextX][nextY], dp[x][y] + 1);
            q1.push({nextX,nextY}); // 将下一步加入队列
...
int main(){
    ...
    for(int i = 0; i < N; i++)
        for(int j = 0; j < N; j++)
            dp[i][j] = INT_MAX;
    dp[start.first][start.second] = 0; // 起点代价为0

    bfs(n,m);
    cout << dp[n][m] << endl; // 注意输出的下标应该是 n-1 和 m-1
    return 0;
}
```
